...
class SBPGenerator
	def initialize()
		...
		@sbpFields = ['Code', 'Date', 'Time', 'Tracking Number', 'Event Code', 'Entry Facility ZIP', 'Space 1', 'Destination ZIP Code', 'Space 2', 'Weight', 'Length', 'Height', 'Width', 'Dim Weight', 'Overlabel']
		@mailClasses = []
		@allDetails = []
		@domClasses = []
		@intClasses = []
		...
		@shortDate = Time.now.strftime('%m%d')
	...
	def selectSTC()
		...
		@eligible = false if /Parcel Return Service/.match(validSTCs[selection]) != nil
	...
	def fileNameGen(type)
		...
		@fileName = "#{$targetPath}\\Generated SBP Files\\PTS-SBP-Extract-#{@shortDate}_#{@stc}" if type == 'sbp'
	end
	#*********************************************************************************************************************************
	...
	def sbpDetailGen()
		...
		if @isManifestBased
			@allDetails.each do |detailRecord|
				...
				lineContents << ''.rjust(7,' ') #7-char white space
				lineContents << ''.rjust(7,' ') #7-char white space
				lineContents << ''.rjust(34,' ') if not ['MA','MR'].include?(@eventCode) #34-char white space
				lineContents << @headerVals['Electronic File Number'].rjust(34,' ') if ['MA','MR'].include?(@eventCode) #If Event Code is MA or MR, include EFN in this field.
		...
		lines.each do |line|
			@allDetails.each do |detail|
				if line['Tracking Number'].include?(detail['Tracking Number'])
					...
					dimWeight = posSize(detail['Dimensional Weight'])
					line['Weight'] = "\"#{weight}\""
					line['Length'] = "\"#{length}\""
					line['Height'] = "\"#{height}\""
					line['Width'] = "\"#{width}\""
					line['Dim Weight'] = "\"#{dimWeight}\""
					line['Event Code'] = "\"03\"" #Event Code must be '03' for POS Samples
