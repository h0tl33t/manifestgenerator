...
require_relative 'rateCheck'
	...
	if $targetPath != '.'
		Dir.mkdir("#{$targetPath}\\SBP Files\\") if File.directory?("#{$targetPath}\\SBP Files\\") != true
	end
	#*********************************************************************************************************************************
...
class SBPGenerator
	...
	def fileNameGen(type)
		@fileName = "#{$targetPath}\\SBP Files\\autogenerated_#{@date}_#{@time}_#{@stc}_SBP" if type == 'manifest'
	...
	def buildSTATSv2()
		...
		statsFile = File.open("#{$targetPath}\\SBP Files\\STATS_#{@date}#{@time}SBP_#{@stc}.DAT", 'w')
		lines.each do |line|
			statsFile.write("\n") if line != lines[0]
			statsFile.write(line)
		end
		statsSem = File.open("#{$targetPath}\\SBP Files\\STATS_#{@date}#{@time}SBP_#{@stc}.sem", 'w')
		statsSem.close()
	...
	def buildPASS()
		...
		passFile = File.open("#{$targetPath}\\SBP Files\\TRP_P1SBP_OUT_#{@date}STC#{@stc}.pass", 'w')
		lines.each do |line|
			passFile.write("\n") if line != lines[0]
			passFile.write(line)
		end
		passSem = File.open("#{$targetPath}\\SBP Files\\TRP_P1SBP_OUT_#{@date}STC#{@stc}.sem", 'w')
		passSem.close()
